...
export function validateFarm(data) {
  ...
  for (const p of data.plantings) {
    ...
    if ((r.tarikhTuaianSebenar && !date(r.tarikhTuaianSebenar)) ||
        ['benihDigunakan', 'bajaDigunakan', 'airDigunakan', 'kosSebenar', 'hasilSebenar', 'hargaJualSebenar'].some(k => r[k] != null && !finite(r[k])) ||
        (r.catatan != null && typeof r.catatan !== 'string')) throw new Error('Rekod operasi tidak sah.');
  }
  return data;
}
const normalize = data => ({ ...data, crops: data.crops.map(c => ({ kaedahTanam: 'terus', tempohSemaian: 0, jenisTuaian: 'sekali', tempohProduktif: 0, ...c })) });
  ...
  function read() {
    ...
    try {
      if (raw !== null) {
        ...
        return { ...saved, data: normalize(saved.data) };
      }
      ...
      return { version: 1, revision: 0, legacy, data: normalize(validateFarm(data)) };
    } catch (error) {
