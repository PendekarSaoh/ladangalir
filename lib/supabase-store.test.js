...
test('the server checks values only for records the request writes', () => {
  const current = saved();
  const old = { id: 'old', cropId: 'c', plotId: 'p', tarikhTanam: '2026-09-20', tarikhTuaianDijangka: '2026-10-15', rekod: { hasilSebenar: -2 } };
  current.data.plantings = [old];
  const base = { requestId: id(), revision: 1 };
  assert.doesNotThrow(() => validateChange(current, { ...base, data: { ...data(), plantings: [old] } }, '2026-09-15'));
  assert.throws(() => validateChange(current, { ...base, data: { ...data(), plantings: [{ ...old, rekod: { hasilSebenar: -5 } }] } }, '2026-09-15'), /negatif/);
  assert.doesNotThrow(() => validateChange(current, { ...base, data: { ...data(), plantings: [{ ...old, rekod: { hasilSebenar: 5 } }] } }, '2026-09-15'));
});
test('a restore may replace the document, but only at the current revision', () => {
  const current = saved();
  const item = { id: 'a', cropId: 'c', plotId: 'p', tarikhTanam: '2026-09-01', tarikhTuaianDijangka: '2026-09-26', rekod: {} };
  current.data.plantings = [item];
  const restored = { ...data(), plots: [{ id: 'p', nama: 'Petak 1' }, { id: 'q', nama: 'Petak 2' }], plantings: [{ ...item, tarikhTanam: '2026-08-01', tarikhTuaianDijangka: '2026-08-26' }] };
  const body = { kind: 'restore', revision: 1, requestId: id(), data: restored };
  assert.doesNotThrow(() => validateChange(current, body, '2026-09-15'));
  assert.throws(() => validateChange(current, { ...body, revision: 0 }, '2026-09-15'), /telah berubah/);
  // The ordinary save path must still refuse to rewrite an existing schedule.
  assert.throws(() => validateChange(current, { ...body, kind: 'save' }, '2026-09-15'), /tidak boleh diubah/);
  assert.doesNotThrow(() => validateChange(null, { ...body, revision: 0 }, '2026-09-15'));
});
test('the client store accepts a restore over existing data and keeps import one-shot', async () => {
  let cloud = saved();
  const store = createSupabaseStore({ getToken: async () => 'token', fetchImpl: async (_url, options) => {
    ...
    const body = JSON.parse(options.body); cloud = { ...cloud, revision: cloud.revision + 1, data: body.data }; return response(cloud);
  } });
