...
test('an old record that predates these rules cannot lock the app out of its own data', async () => {
  const plantings = [
    { ...planting('old'), rekod: { hasilSebenar: -4 } },
    { ...planting('older'), rekod: { tarikhTuaianSebenar: '2026-01-01' } },
  ];
  const stored = JSON.stringify({ version: 1, revision: 3, legacy: [null, null, null], data: { ...seed(), plantings } });
  const f = fixture({ [STORE_KEY]: stored });
  const store = f.connect();
  const loaded = await store.initialize();
  assert.equal(loaded.data.plantings.length, 2);
  // An unrelated edit still saves while the grandfathered records stay untouched.
  await store.transact(data => ({ ...data, plots: [...data.plots, { id: 'r', nama: 'Petak 3' }] }));
  assert.equal(store.read().data.plots.length, 3);
  // Fixing one of them is allowed, and a new invalid value is still refused.
  await store.transact(data => ({ ...data, plantings: [{ ...plantings[0], rekod: { hasilSebenar: 3 } }, plantings[1]] }));
  await assert.rejects(store.transact(data => ({ ...data, plantings: [{ ...plantings[0], rekod: { kosSebenar: -1 } }, plantings[1]] })), /negatif/);
});
test('records that would skew the reports are rejected before they are saved', async () => {
  const f = fixture(); const store = f.connect(); await store.initialize(); const before = f.values.get(STORE_KEY);
  // planting('a') is planted on 2026-09-20, so a harvest before that date is impossible.
  const invalid = [
    { ...planting('a'), rekod: { tarikhTuaianSebenar: '2026-09-19' } },
    { ...planting('a'), rekod: { hasilSebenar: -1 } },
    { ...planting('a'), rekod: { kosSebenar: -0.01 } },
    { ...planting('a'), rekod: { hargaJualSebenar: -3 } },
  ];
  for (const record of invalid) {
    await assert.rejects(store.transact(data => ({ ...data, plantings: [record] })));
    assert.equal(f.values.get(STORE_KEY), before);
  }
  // The same day and zero are real values, not errors.
  await store.transact(data => ({ ...data, plantings: [{ ...planting('a'), rekod: { tarikhTuaianSebenar: '2026-09-20', hasilSebenar: 0, kosSebenar: 0 } }] }));
  assert.equal(store.read().data.plantings[0].rekod.hasilSebenar, 0);
});
