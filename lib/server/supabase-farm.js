...
export function validateChange(current, body, today) {
  ...
  if (!current) {
    if ((body.kind !== 'import' && body.kind !== 'restore') || body.revision !== 0) throw new FarmApiError('Import atau mulakan kebun dahulu.', 409);
    return;
  }
  ...
  // A restore replaces the whole document on purpose. It still needs the matching revision
  // above, so a stale file cannot silently overwrite newer work. The log-only guard below is
  // an accident guard for the ordinary save path, not an authorisation boundary.
  if (body.kind === 'restore') return;
  const original = new Map(current.data.plantings.map(p => [p.id, p]));
  // Only records this request adds or edits are checked; untouched history is grandfathered.
  validateRecordValues(changedPlantings(current.data.plantings, body.data.plantings));
  const additions = [];
